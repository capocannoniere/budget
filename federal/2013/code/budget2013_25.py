...
class JsonEncoder_Budget2013_25(json.JSONEncoder):
	def default(self, o):
		return {
			"caption": o.caption,
			"header": o.header,
...
def check_document(document):
	...
	if not numbers_equal(total, document.data[-1]["value"]):
		print total, document.data[-1]["value"]
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8") as input_file:
		...
		while line_index < len(input_data):
			...
			value = m.group(1)
			name = data_line[:len(data_line) - len(value)].strip()
			item = {"name": name, "value": float(value.replace(',', '.').replace(' ', ''))}
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_25)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_25)
