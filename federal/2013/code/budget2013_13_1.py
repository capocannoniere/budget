...
def process_item(item):
	...
	regexps = [
		re.compile(r"\s\s+(\d{2})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{2})\s\s+(\d{2})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{2})\s\s+(\d{2})\s\s+(\d{7})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{2})\s\s+(\d{2})\s\s+(\d{7})\s\s+(\d{3})(?:\s\s+(" + value + "))?$")
	]
...
if __name__ == "__main__":
	...
	parser.add_argument("-m", "--main-input", dest="main_input_file_name", required=True, help="Path to the annex13 text file")
	parser.add_argument("-i", "--input", dest="input_file_name", required=True, help="Path to the annex13.1 text file")
	
	parser.add_argument("--output-pickle", dest="output_pickle_file_name", help="Path to the output pickle file for annex13.1")
	parser.add_argument("--output-text", dest="output_text_file_name", help="Path to the output text file for annex13.1")
	parser.add_argument("--output-json", dest="output_json_file_name", help="Path to the output JSON file for annex13.1")
	parser.add_argument("--output-json-pretty", dest="output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for annex13.1")
	
	parser.add_argument("--combined-output-pickle", dest="combined_output_pickle_file_name", help="Path to the output pickle file for combined annex13 + annex13.1")
	parser.add_argument("--combined-output-text", dest="combined_output_text_file_name", help="Path to the output text file for combined annex13 + annex13.1")
	parser.add_argument("--combined-output-json", dest="combined_output_json_file_name", help="Path to the output JSON file for combined annex13 + annex13.1")
	parser.add_argument("--combined-output-json-pretty", dest="combined_output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for combined annex13 + annex13.1")
	...
	main_document = budget2013_13.get_document(main_input_file_name)
