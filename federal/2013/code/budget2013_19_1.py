...
def process_item(item):
	...
	regexps = [
		re.compile(r"\s\s+(\d{7})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{7})\s\s+(\d{2})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{7})\s\s+(\d{2})\s\s+(\d{2})(?:\s\s+(" + value + "))?$"),
		re.compile(r"\s\s+(\d{7})\s\s+(\d{2})\s\s+(\d{2})\s\s+(\d{3})(?:\s\s+(" + value + "))?$")
	]
...
def process_items(items, document):
	for item in items:
		(name, csr, rz, pr, vr, value) = process_item(item)
		
		if name:
			while name.find("  ") != -1:
				name = name.replace("  ", " ")
		
		data_item = budget2013_19.Budget2013_19_DataItem()
		data_item.name = name
		data_item.csr = csr
		data_item.rz = rz
		data_item.pr = pr
...
def patch_item(item, combined_document):
	...
	if not combined_item:
		...
		combined_item = budget2013_19.Budget2013_19_DataItem()
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8") as input_file:
		...
		document = budget2013_19.Budget2013_19()
...
if __name__ == "__main__":
	...
	parser.add_argument("-m", "--main-input", dest="main_input_file_name", required=True, help="Path to the annex19 text file")
	parser.add_argument("-i", "--input", dest="input_file_name", required=True, help="Path to the annex19.1 text file")
	
	parser.add_argument("--output-pickle", dest="output_pickle_file_name", help="Path to the output pickle file for annex19.1")
	parser.add_argument("--output-text", dest="output_text_file_name", help="Path to the output text file for annex19.1")
	parser.add_argument("--output-json", dest="output_json_file_name", help="Path to the output JSON file for annex19.1")
	parser.add_argument("--output-json-pretty", dest="output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for annex19.1")
	
	parser.add_argument("--combined-output-pickle", dest="combined_output_pickle_file_name", help="Path to the output pickle file for combined annex19 + annex19.1")
	parser.add_argument("--combined-output-text", dest="combined_output_text_file_name", help="Path to the output text file for combined annex19 + annex19.1")
	parser.add_argument("--combined-output-json", dest="combined_output_json_file_name", help="Path to the output JSON file for combined annex19 + annex19.1")
	parser.add_argument("--combined-output-json-pretty", dest="combined_output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for combined annex19 + annex19.1")
	...
	if (not output_pickle_file_name) and (not output_text_file_name) and (not output_json_file_name) and (not output_json_pretty_file_name):
		raise Exception("No output file specified for annex19.1")
	...
	if (not combined_output_pickle_file_name) and (not combined_output_text_file_name) and (not combined_output_json_file_name) and (not combined_output_json_pretty_file_name):
		raise Exception("No output file specified for combined annex19 + annex19.1")

	main_document = budget2013_19.get_document(main_input_file_name)
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, budget2013_19.JsonEncoder_Budget2013_19)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, budget2013_19.JsonEncoder_Budget2013_19)
	...
	if combined_output_text_file_name:
		write_text_document(combined_document, combined_output_text_file_name, budget2013_19.do_write_text_document)
	if combined_output_json_file_name:
		write_json_document(combined_document, combined_output_json_file_name, budget2013_19.JsonEncoder_Budget2013_19)
	if combined_output_json_pretty_file_name:
		write_json_pretty_document(combined_document, combined_output_json_pretty_file_name, budget2013_19.JsonEncoder_Budget2013_19)
