...
class Budget2013_41_SubTable1Item(object):
	def __init__(self):
		self._no = None
		self._purpose = None
...
class JsonEncoder_Budget2013_41_SubTable1Item(json.JSONEncoder):
	def default(self, o):
		return {
			"no": o.no,
			"purpose": o.purpose,
...
class Budget2013_41_SubTable1(object):
	def __init__(self):
		self._caption = None
		self._headers = []
		self._items = []
...
class JsonEncoder_Budget2013_41_SubTable1(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_41_SubTable1Item()
		return {
			"caption": o.caption,
			"headers": o.headers,
...
class Budget2013_41_SubTable2(object):
	def __init__(self):
		self._caption = None
		self._headers = []
		self._items = []
...
class JsonEncoder_Budget2013_41_SubTable2Item(json.JSONEncoder):
	def default(self, o):
		return {
			"name": o["name"],
			"value": o["value"]
...
class JsonEncoder_Budget2013_41_SubTable2(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_41_SubTable2Item()
		return {
			"caption": o.caption,
			"headers": o.headers,
			"items": [item_encoder.default(item) for item in o.items]
...
class Budget2013_41_SubTables(object):
	def __init__(self):
		self._caption = None
		self._subtable1 = Budget2013_41_SubTable1()
		self._subtable2 = Budget2013_41_SubTable2()
...
class JsonEncoder_Budget2013_41_SubTables(json.JSONEncoder):
	def default(self, o):
		subtable1_encoder = JsonEncoder_Budget2013_41_SubTable1()
		subtable2_encoder = JsonEncoder_Budget2013_41_SubTable2()
		return {
			"caption": o.caption,
...
class Budget2013_41(object):
	def __init__(self):
		self._caption = None
		self._subtables1 = Budget2013_41_SubTables()
		self._subtables2 = Budget2013_41_SubTables()
		self._subtables3 = Budget2013_41_SubTables()
	...
	@property
	def subtables3(self):
		return self._subtables3

class JsonEncoder_Budget2013_41(json.JSONEncoder):
	def default(self, o):
		subtables_encoder = JsonEncoder_Budget2013_41_SubTables()
		return {
			"caption": o.caption,
			"subtables1": subtables_encoder.default(o.subtables1),
			"subtables2": subtables_encoder.default(o.subtables2),
			"subtables3": subtables_encoder.default(o.subtables3)
		}
...
def check_subtables(subtables):
	...
	if total_value != subtables.subtable1.items[-1].value:
		print subtables.subtable1.caption, total_value, subtables.subtable1.items[-1].value
...
def correct_document(document):
	...
	correct_purpose(document.subtables3.subtable1)
	
	parent_item = document.subtables2.subtable1.items[0]
	item = Budget2013_41_SubTable1Item()
...
def check_document(document):
	...
	check_subtables(document.subtables3)
...
def read_subtables(subtables, line_index, input_data):
	...
	headers = input_data[line_index].strip()
	line_index += 2
	subtables.subtable2.headers = headers.split(";")
	...
	while line_index < len(input_data):
		...
		name = data_line[:-len(m.group(1))].strip()
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8-sig") as input_file:
		...
		line_index = read_subtables(document.subtables3, line_index, input_data)
...
def write_item(output_file, item, level = 0):
	output_file.write(" " * level + (unicode(item.no) + " " if item.no else "") + item.purpose + " " + 
		(item.principal + " " if item.principal else "") + (unicode(item.value) + " " if item.value else "") + 
		(unicode(item.regress) + " " if item.regress else "") + (unicode(item.check) if item.check else "") + "\r\n")
	if item.other:
		for o in item.other:
...
def write_subtables(output_file, subtables):
	...
	output_file.write(subtables.subtable1.items[-1].purpose + " " + unicode(subtables.subtable1.items[-1].value) + "\r\n\r\n")
...
def do_write_text_document(output_file, document):
	...
	write_subtables(output_file, document.subtables3)
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_41)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_41)
