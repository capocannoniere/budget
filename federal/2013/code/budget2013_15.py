...
class Budget2013_15_DataItem(object):
	def __init__(self):
		self._name = None
		self._rz = None
		self._pr = None
...
class JsonEncoder_Budget2013_15_DataItem(json.JSONEncoder):
	def default(self, o):
		return {
			"name": o.name,
			"rz": o.rz,
...
class JsonEncoder_Budget2013_15(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_15_DataItem()
		return {
			"caption": o.caption,
			"header": o.header,
...
def parse_item_data(item_data):
	...
	regexps = [
		re.compile("^(\\d{2})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{2}) (\\d{2})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{2}) (\\d{2}) (\\d{7})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{2}) (\\d{2}) (\\d{7}) (\\d{3})  " + value_regexp + " " + value_regexp + "$")
	]
...
def parse_item(item_name, item_data):
	(rz, pr, csr, vr, value2014, value2015) = parse_item_data(item_data)
	
	item = Budget2013_15_DataItem()
	item.name = item_name
	item.rz = rz
	item.pr = pr
	item.csr = csr
...
#	print u"Page ", page_no
	...
	if data_in_the_end:
		...
		r1 = re.compile("\\s+((?:\\d{2} )?(?:\\d{2} )?(?:\\d{7} )?(?:\\d{3} )? " + regexp_value + " " + regexp_value + ")$")
		item_lines = []
		for page_line in page:
...
def get_item_text(item):
	return (item.rz + " " if item.rz else "") + (item.pr + " " if item.pr else "") + (item.csr + " " if item.csr else "") + (item.vr + " " if item.vr else "") + item.name + " " + unicode(item.value2014) + " " + unicode(item.value2015)
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_15)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_15)
