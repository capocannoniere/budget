...
class Budget2013_22_DataItem(object):
	def __init__(self):
		self._name = None
		self._csr = None
		self._rz = None
...
class JsonEncoder_Budget2013_22_DataItem(json.JSONEncoder):
	def default(self, o):
		return {
			"name": o.name,
			"csr": o.csr,
...
class JsonEncoder_Budget2013_22(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_22_DataItem()
		return {
			"caption": o.caption,
			"header": o.header,
...
def parse_item_data(item_data):
	...
	regexps = [
		re.compile("^(\\d{7})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{7}) (\\d{2})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{7}) (\\d{2}) (\\d{2})  " + value_regexp + " " + value_regexp + "$"),
		re.compile("^(\\d{7}) (\\d{2}) (\\d{2}) (\\d{3})  " + value_regexp + " " + value_regexp + "$")
	]
	...
	if success_count == 0:
		...
		if results[0]:
			m = results[0]
			return (m.group(1), None, None, None, parse_value(m.group(2)), parse_value(m.group(3)))
		elif results[1]:
			m = results[1]
			return (m.group(1), m.group(2), None, None, parse_value(m.group(3)), parse_value(m.group(4)))
		elif results[2]:
			m = results[2]
			return (m.group(1), m.group(2), m.group(3), None, parse_value(m.group(4)), parse_value(m.group(5)))
		elif results[3]:
			m = results[3]
			return (m.group(1), m.group(2), m.group(3), m.group(4), parse_value(m.group(5)), parse_value(m.group(6)))
...
def parse_item(item_name, item_data):
	(csr, rz, pr, vr, value2014, value2015) = parse_item_data(item_data)
	
	item = Budget2013_22_DataItem()
	item.name = item_name
	item.csr = csr
	item.rz = rz
	item.pr = pr
...
def process_lines(lines, document):
	...
	r1 = re.compile("((?:\\d{7} )(?:\\d{2} )?(?:\\d{2} )?(?:\\d{3} )? \\d\\d?\\d?(?: \\d{3})*,\\d+ \\d\\d?\\d?(?: \\d{3})*,\\d+)$")
	item_lines = []
	for line in lines:
		m_total = re.compile(u"^(ВСЕГО) (.*,\\d+) (.*,\\d+)").match(line)
		if m_total:
			if item_lines:
...
def get_item_text(item):
	return (item.csr + " " if item.csr else "") + (item.rz + " " if item.rz else "") + (item.pr + " " if item.pr else "") + (item.vr + " " if item.vr else "") + item.name + " " + unicode(item.value2014) + " " + unicode(item.value2015)
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8") as input_file:
		...
		document.header = headers[:-1]
		last_headers = headers[-1].split(":")
		last_headers_years = last_headers[1].split(",")
		document.header.append(last_headers[0] + ":" + last_headers_years[0])
		document.header.append(last_headers[0] + ":" + last_headers_years[1])
...
def write_data_item(output_file, item, level = 0):
	output_file.write("\t" * level + (item.csr + " " if item.csr else "") +
		...
		(item.vr + " " if item.vr else "") + item.name + " " + unicode(item.value2014) + " " + unicode(item.value2015) + "\r\n")
	for child in item.children:
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_22)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_22)
