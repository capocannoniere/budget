...
def process_item(item):
	...
	regexps = [
		re.compile(r"\s+(\d{3})(?:\s+(" + value + ")?)?$"),
		re.compile(r"\s+(\d{3})\s+(\d{2})(?:\s\s+(" + value + ")?)?$"),
		re.compile(r"\s+(\d{3})\s+(\d{2})\s+(\d{2})(?:\s+(" + value + ")?)?$"),
		re.compile(r"\s+(\d{3})\s+(\d{2})\s+(\d{2})\s+(\d{7})(?:\s+(" + value + ")?)?$"),
		re.compile(r"\s+(\d{3})\s+(\d{2})\s+(\d{2})\s+(\d{7})\s+(\d{3})(?:\s+(" + value + ")?)?$")
	]
	...
	if success_count == 2 and results[0] and results[4]:
		results[0] = None
		success_count = 1
	...
	if success_count == 0:
		...
		if m:
			...
			if name == u"ВСЕГО":
				return (name, None, None, None, None, None, parse_value(m.group(1)) if m.group(1) else 0.0)
		print item
		raise Exception(u"No match for item data '{0}'".format(item))
	elif success_count == 1:
		if results[0]:
			return (item[:-len(results[0].group(0))].strip(), results[0].group(1), None, None, None, None, parse_value(results[0].group(2)) if results[0].group(2) else 0.0)
		elif results[1]:
			return (item[:-len(results[1].group(0))].strip(), results[1].group(1), results[1].group(2), None, None, None, parse_value(results[1].group(3)) if results[1].group(3) else 0.0)
		elif results[2]:
			return (item[:-len(results[2].group(0))].strip(), results[2].group(1), results[2].group(2), results[2].group(3), None, None, parse_value(results[2].group(4)) if results[2].group(4) else 0.0)
		elif results[3]:
			return (item[:-len(results[3].group(0))].strip(), results[3].group(1), results[3].group(2), results[3].group(3), results[3].group(4), None, parse_value(results[3].group(5)) if results[3].group(5) else 0.0)
		elif results[4]:
			return (item[:-len(results[4].group(0))].strip(), results[4].group(1), results[4].group(2), results[4].group(3), results[4].group(4), results[4].group(5), parse_value(results[4].group(6)) if results[4].group(6) else 0.0)
...
def process_items(items, document):
	for item in items:
		(name, min, rz, pr, csr, vr, value) = process_item(item)
		
		if name:
			while name.find("  ") != -1:
				name = name.replace("  ", " ")
		
		data_item = budget2013_7.Budget2013_7_DataItem()
		data_item.name = name
		data_item.min = min
		data_item.rz = rz
		data_item.pr = pr
...
def check_item_values(item):
	if item.children:
		...
		if not numbers_equal(item.value, children_value):
			print u"Values are NOT equal: ({0}), value {1} and its children, value {2}, diff={3}.".format(
				get_item_short_name(item), item.value, children_value, item.value - children_value)
		for child in item.children:
...
def find_item(item, combined_document):
	...
	if not item.min:
		return current_item
	for child in current_item.children:
		if child.min == item.min:
			current_item = child
			break
	if current_item.min != item.min:
		return None
...
def patch_item(item, combined_document):
	...
	if not combined_item:
		...
		combined_item = budget2013_7.Budget2013_7_DataItem()
		combined_item.name = item.name
		combined_item.min = item.min
...
def correct_document_items(document, min, rz, pr, csr, vr, child_vrs, name):
	item = budget2013_7.Budget2013_7_DataItem()
	item.min = min
	item.rz = rz
	item.pr = pr
	item.csr = csr
	...
	new_item = budget2013_7.Budget2013_7_DataItem()
	new_item.min = min
	new_item.rz = rz
	new_item.pr = pr
	new_item.csr = csr
...
def correct_document(document):
	correct_document_items(document, "177", "07", "05", "4299900", "610", ["611", "612"], u"Субсидии бюджетным учреждениям")
	correct_document_items(document, "204", "03", "13", "0819900", "110", ["111", "112"], u"Расходы на выплаты персоналу казенных учреждений")
	correct_document_items(document, "204", "07", "06", "4309900", "110", ["111", "112"], u"Расходы на выплаты персоналу казенных учреждений")

def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8") as input_file:
		input_data = input_file.readlines()
		
		document = budget2013_7.Budget2013_7()
...
if __name__ == "__main__":
	...
	parser.add_argument("-m", "--main-input", dest="main_input_file_name", required=True, help="Path to the combined annex07 + annex07.1 pickle file")
	parser.add_argument("-i", "--input", dest="input_file_name", required=True, help="Path to the annex07.2 text file")
	
	parser.add_argument("--output-pickle", dest="output_pickle_file_name", help="Path to the output pickle file for annex07.2")
	parser.add_argument("--output-text", dest="output_text_file_name", help="Path to the output text file for annex07.2")
	parser.add_argument("--output-json", dest="output_json_file_name", help="Path to the output JSON file for annex07.2")
	parser.add_argument("--output-json-pretty", dest="output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for annex07.2")
	
	parser.add_argument("--combined-output-pickle", dest="combined_output_pickle_file_name", help="Path to the output pickle file for combined annex07 + annex07.1 + annex07.2")
	parser.add_argument("--combined-output-text", dest="combined_output_text_file_name", help="Path to the output text file for combined annex07 + annex07.1 + annex07.2")
	parser.add_argument("--combined-output-json", dest="combined_output_json_file_name", help="Path to the output JSON file for combined annex07 + annex07.1 + annex07.2")
	parser.add_argument("--combined-output-json-pretty", dest="combined_output_json_pretty_file_name", help="Path to the output JSON file with indents and russian letters for combined annex07 + annex07.1 + annex07.2")
	...
	if (not output_pickle_file_name) and (not output_text_file_name) and (not output_json_file_name) and (not output_json_pretty_file_name):
		raise Exception("No output file specified for annex07.1")
	...
	if (not combined_output_pickle_file_name) and (not combined_output_text_file_name) and (not combined_output_json_file_name) and (not combined_output_json_pretty_file_name):
		raise Exception("No output file specified for combined annex07 + annex07.1")
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, budget2013_7.JsonEncoder_Budget2013_7)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, budget2013_7.JsonEncoder_Budget2013_7)
	...
	if combined_output_text_file_name:
		write_text_document(combined_document, combined_output_text_file_name, budget2013_7.do_write_text_document)
	if combined_output_json_file_name:
		write_json_document(combined_document, combined_output_json_file_name, budget2013_7.JsonEncoder_Budget2013_7)
	if combined_output_json_pretty_file_name:
		write_json_pretty_document(combined_document, combined_output_json_pretty_file_name, budget2013_7.JsonEncoder_Budget2013_7)
