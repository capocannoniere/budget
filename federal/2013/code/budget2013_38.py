...
class Budget2013_38_SubTable1Item(object):
	def __init__(self):
		self._no = None
		self._purpose = None
...
class JsonEncoder_Budget2013_38_SubTable1Item(json.JSONEncoder):
	def default(self, o):
		return {
			"no": o.no,
			"purpose": o.purpose,
...
class Budget2013_38_SubTable1(object):
	def __init__(self):
		self._caption = None
		self._headers = []
		self._items = []
...
class JsonEncoder_Budget2013_38_SubTable1(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_38_SubTable1Item()
		return {
			"caption": o.caption,
			"headers": o.headers,
...
class Budget2013_38_SubTable2(object):
	def __init__(self):
		self._caption = None
		self._headers = []
		self._items = []
...
class JsonEncoder_Budget2013_38_SubTable2Item(json.JSONEncoder):
	def default(self, o):
		return {
			"name": o["name"],
			"value2014": o["value2014"],
...
class JsonEncoder_Budget2013_38_SubTable2(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_38_SubTable2Item()
		return {
			"caption": o.caption,
			"headers": o.headers,
			"items": [item_encoder.default(item) for item in o.items]
...
class Budget2013_38(object):
	def __init__(self):
		self._caption = None
		self._subtable1 = Budget2013_38_SubTable1()
		self._subtable2 = Budget2013_38_SubTable2()
...
class JsonEncoder_Budget2013_38(json.JSONEncoder):
	def default(self, o):
		subtable1_encoder = JsonEncoder_Budget2013_38_SubTable1()
		subtable2_encoder = JsonEncoder_Budget2013_38_SubTable2()
		return {
			"caption": o.caption,
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8-sig") as input_file:
		...
		while not input_data[line_index].strip().startswith(u"ИТОГО"):
			item = Budget2013_38_SubTable1Item()
		...
		# ИТОГО
		s = input_data[line_index].strip()
		m = re.compile(u"^(ИТОГО)\\*? ([ \\d]+,\\d+) ([ \\d]+,\\d+)").match(s)
		item = Budget2013_38_SubTable1Item()
		...
		document.subtable2.headers = headers[:-1]
		last_header = headers[-1].split(":")
		last_header_years = last_header[1].split(",")
		document.subtable2.headers.append(last_header[0] + " " + last_header_years[0])
		document.subtable2.headers.append(last_header[0] + " " + last_header_years[1])
		...
		while line_index < len(input_data):
			...
			if not data_line:
				break
			m = re.compile("([ \\d]+,\\d+) ([ \\d]+,\\d+)$").search(data_line)
			value2014 = float(m.group(1).replace(",", ".").replace(" ", ""))
			value2015 = float(m.group(2).replace(",", ".").replace(" ", ""))
			name = data_line[:len(data_line) - len(m.group(1)) - 1 - len(m.group(2))].strip()
...
def do_write_text_document(output_file, document):
	...
	for item in document.subtable1.items[:-1]:
		output_file.write(unicode(item.no) + " " + item.purpose + " " + 
			item.principal + " " + unicode(item.value2014) + " " + unicode(item.value2015) + " " + 
			unicode(item.regress) + " " + unicode(item.check) + "\r\n")
		if item.other:
			for o in item.other:
				output_file.write(o + "\r\n");
		output_file.write("\r\n")
	output_file.write(document.subtable1.items[-1].purpose + " " + unicode(document.subtable1.items[-1].value2014) + " " + 
		unicode(document.subtable1.items[-1].value2015)	+ "\r\n\r\n")
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_38)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_38)
