...
class JsonEncoder_Budget2013_3(json.JSONEncoder):
	def default(self, o):
		return {
			"caption": o.caption,
			"header": o.header,
...
def get_document(input_file_name):
	with codecs.open(input_file_name, "r", encoding = "utf-8") as input_file:
		...
		while line_index < len(input_data):
			...
			if not data_line:
				break
			r = re.compile("(.*)\\s+(\\d+(?:,\\d+)?\\*?)\\s+(\\d+(?:,\\d+)?\\*?)")
			m = r.match(data_line)
			item = {"name": m.group(1), "value2014": float(m.group(2).replace(',', '.')), "value2015": float(m.group(3).replace(',', '.'))}
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_3)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_3)
