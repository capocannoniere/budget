...
class Budget2013_7_DataItem(object):
	def __init__(self):
		self._name = None
		self._min = None
		self._rz = None
...
class JsonEncoder_Budget2013_7_DataItem(json.JSONEncoder):
	def default(self, o):
		return {
			"name": o.name,
			"min": o.min,
...
class JsonEncoder_Budget2013_7(json.JSONEncoder):
	def default(self, o):
		item_encoder = JsonEncoder_Budget2013_7_DataItem()
		return {
			"caption": o.caption,
			"header": o.header,
...
def parse_item_data(item_data):
	regexps = [
		re.compile("^(\\d{3})  ((?:(?:[1-9](?: \\d{3})*)|0),\\d\\d?)$"),
		re.compile("^(\\d{3})  ([1-9]\\d(?: \\d{3})*,\\d\\d?)$"),
		re.compile("^(\\d{3})  ([1-9]\\d{2}(?: \\d{3})*,\\d\\d?)$"),

		re.compile("^(\\d{3}) (\\d{2})  ((?:(?:[1-9](?: \\d{3})*)|0),\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2})  ([1-9]\\d(?: \\d{3})*,\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2})  ([1-9]\\d{2}(?: \\d{3})*,\\d\\d?)$"),

		re.compile("^(\\d{3}) (\\d{2}) (\\d{2})  ((?:(?:[1-9](?: \\d{3})*)|0),\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2})  ([1-9]\\d(?: \\d{3})*,\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2})  ([1-9]\\d{2}(?: \\d{3})*,\\d\\d?)$"),

		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7})  ((?:(?:[1-9](?: \\d{3})*)|0),\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7})  ([1-9]\\d(?: \\d{3})*,\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7})  ([1-9]\\d{2}(?: \\d{3})*,\\d\\d?)$"),

		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7}) (\\d{3})  ((?:(?:[1-9](?: \\d{3})*)|0),\\d\\d?)$"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7}) (\\d{3})  ([1-9]\\d(?: \\d{3})*,\\d\\d?)"),
		re.compile("^(\\d{3}) (\\d{2}) (\\d{2}) (\\d{7}) (\\d{3})  ([1-9]\\d{2}(?: \\d{3})*,\\d\\d?)")
	]
	...
	if success_count == 0:
		...
		if results[0]:
			...
			m = results[2]
		if m:
			return (m.group(1), None, None, None, None, parse_value(m.group(2)))
		...
		if results[3]:
			...
			m = results[5]
		if m:
			return (m.group(1), m.group(2), None, None, None, parse_value(m.group(3)))
		...
		if results[6]:
			...
			m = results[8]
		if m:
			return (m.group(1), m.group(2), m.group(3), None, None, parse_value(m.group(4)))
		...
		if results[9]:
			...
			m = results[11]
		if m:
			return (m.group(1), m.group(2), m.group(3), m.group(4), None, parse_value(m.group(5)))
		...
		if results[12]:
			...
			m = results[14]
		if m:
			return (m.group(1), m.group(2), m.group(3), m.group(4), m.group(5), parse_value(m.group(6)))
	...
	return (min, rz, pr, csr, vr, value)

def parse_item(item_name, item_data):
	(min, rz, pr, csr, vr, value) = parse_item_data(item_data)
	
	item = Budget2013_7_DataItem()
	item.name = item_name
	item.min = min
	item.rz = rz
	item.pr = pr
...
#	print u"Page ", page_no
	...
	if data_in_the_end:
		...
		r1 = re.compile("\\s+(\\d{3} (?:\\d{2} )?(?:\\d{2} )?(?:\\d{7} )?(?:\\d{3} )? \\d\\d?\\d?(?: \\d{3})*,\\d\\d?)$")
		item_lines = []
		for page_line in page:
...
def get_item_text(item):
	return (item.min + " " if item.min else "") + (item.rz + " " if item.rz else "") + (item.pr + " " if item.pr else "") + (item.csr + " " if item.csr else "") + (item.vr + " " if item.vr else "") + item.name + " " + unicode(item.value)
...
if __name__ == "__main__":
	...
	if output_json_file_name:
		write_json_document(document, output_json_file_name, JsonEncoder_Budget2013_7)
	if output_json_pretty_file_name:
		write_json_pretty_document(document, output_json_pretty_file_name, JsonEncoder_Budget2013_7)
